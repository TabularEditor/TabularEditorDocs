...
// It creates a new measure [RefDate] and a 'Date' table 
//    The columns are organized in display folders
//    The columns are configured (i.e. Sort By)
//
// Use this script when you want to create a new, formatted date table in your model, based on existing Date fields.
//
// Config: Run script when selecting 1-2 columns with the (1) Earliest & (2) Latest dates
// Get column names
var _ColumnsList = new List<string>();
string _EarliestDate = "";
string _LatestDate = "";

if ( Selected.Columns.Count() >= 2 )
{
    ...
    if ( Selected.Columns.Count() == 2 )
    {
        _EarliestDate = _ColumnsList[0];
        _LatestDate = _ColumnsList[1];
    }

    else
    {
        Error ( "Please select two columns; the earliest & latest dates in the model." );
    }
}

if ( Selected.Columns.Count() == 1 )
{
    _EarliestDate = Selected.Column.DaxObjectFullName;
    _LatestDate = Selected.Column.DaxObjectFullName;
}

//-------------------------------------------------------------------------------------------//

// Create measure for reference date
var _RefDateMeasure = Selected.Table.AddMeasure(
    "RefDate",
...
## Example Output
<br>
<img src="~/images/Cscripts/script-create-date-table.png" alt="Image description" id="create-date-table">
<script>
    var img = document.getElementById("create-date-table");
