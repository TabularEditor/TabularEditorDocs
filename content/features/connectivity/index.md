...
Whichever kind the model ends up with, the credentials *Tabular Editor* uses to browse the source and read its schema are the ones you type into the connection dialog, and they go in the `.tmuo` file described above. They are separate from the credentials Analysis Services or Power BI uses at refresh time.
