...
| **`Info` / `Warning` / `Error` / `Output` write to the console** | These still work, but route to stdout/stderr instead of opening a dialog. They never block and never offer an "ignore further popups" prompt. Safe to use in CI. A script that calls `Error(...)` makes `te script` exit non-zero (changes are still saved with `--save`); `Warning` and `Info` do not. |
| **Relationship names cannot be assigned** | A relationship's name always describes the columns it connects. Assigning `Relationship.Name` in a script stops the script with a message saying so (previously it silently did nothing), and `te move` refuses to rename a relationship for the same reason. |
