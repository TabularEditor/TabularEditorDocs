...
| **`bpa.rules` config and `TE_BPA_PATH` are not currently honored by `te bpa run`** | Setting `bpa.rules` via [`te config set`](xref:te-cli-commands#config-show--paths--init--set) is accepted (and confirmed by `te config show`), but `te bpa run` currently loads only built-in rules and model annotations - the configured path and the `TE_BPA_PATH` environment variable are ignored. Pass the rule file explicitly with `--rules <file-or-url>` until this is fixed. |
