...
Check out our [release blog](https://tabulareditor.com/blog/tabular-editor-3-september-2026-release) to get a brief overview of the most important updates in this release.
...
Choosing a model is easier. The **AI model name** dropdown and the model picker in the panel are filled from a curated list maintained online and refreshed about once a day, with the recommended model for the provider listed first, so new models become selectable without waiting for a Tabular Editor update; both fields remain free text. The context usage bar, the point at which a conversation is compacted and the maximum length of a reply now follow the real limits of the model in use rather than one fixed figure, and with **Azure OpenAI** the model field is labeled **Deployment**, as Azure does. Thank you to [edhans](https://github.com/edhans) for suggesting the model picker on our [GitHub ideas forum](https://github.com/TabularEditor/TabularEditor3/discussions/1580).
...
The behavior is enabled by default and can be turned off with the new **Automatically reload from disk (hot reload)** checkbox under **Tools > Preferences > Miscellaneous**. For details, see the new @auto-reload article.
...
These policies require Tabular Editor 3 Enterprise Edition. Other editions refuse to start the AI Assistant and the MCP server while these policies are set, and name the values that need the Enterprise Edition. For details, see [Enterprise policies](xref:policies#enterprise-policies).
...
- Selecting a column in the **TOM Explorer** now scrolls the **Data Preview** to that column and highlights it, so you no longer have to scroll across a wide table to find it. Tracking is on by default and can be turned off for an individual preview via the **Track selected column** toolbar button, or globally under **Tools > Preferences > Data Browsing > Table Preview > Behavior**. See [Finding a column in a wide table](xref:table-preview#finding-a-column-in-a-wide-table).
...
- The DAX editor now recognizes the optional `AssociatedColumnsBehavior` parameter of the [LOOKUP](https://dax.guide/lookup/) function, which was previously underlined as an unresolved reference. It appears in the calltip as an optional trailing argument, and auto-complete offers its two values, `EXPLICIT` and `INFERRED`, cased according to your **Preferred keyword casing** preference.
