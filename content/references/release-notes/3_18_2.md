...
- The Semantic Analyzer no longer displays an error when using a table name as the name of a variable. **Note:** as of this writing, the ability to use table names for variables is supported in Power BI Desktop (October 2024) and in the Power BI Service. You can control the error behavior under **Tools > Preferences > DAX Editor > DAX Settings > Table-named variables**.
