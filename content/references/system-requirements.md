...
| Runtime | Architecture | Formats |
|---|---|---|
| .NET 10 *(recommended)* | x64, ARM64 | `.exe` installer, `.msi`, portable `.zip` |
| .NET 8 | x64, ARM64 | `.exe` installer, `.msi`, portable `.zip` |

The .NET 10 and .NET 8 builds are *functionally identical*. Nothing is available in one and missing from the other, and the two can be installed side by side.
...
| `.msi` | The matching .NET Desktop Runtime, installed beforehand. The MSI does *not* bring it along, which is what makes it suitable for unattended deployment |
| Portable `.zip` | Nothing. It is self-contained |

It must be the *Desktop* runtime. The ASP.NET Core runtime and the plain .NET runtime do not carry the Windows Forms and WPF libraries the application needs.
