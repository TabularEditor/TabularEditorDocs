...
When this article says "Octopus Merge," it means (3). Note that (3) *uses* the native strategy from (1) as its actual merge mechanism — the value it adds is the automation and branch lifecycle around that merge, not an alternative way of merging.
...
The pipeline isn't triggered by a normal Git push trigger. Since it needs to merge *all* currently open pull requests — not just the one that changed — it's typically set up with no automatic branch trigger, and instead invoked in one of two ways:
...
- @cicd-overview — the CI/CD for Semantic Models section this pipeline pattern belongs to.
- @powerbi-cicd
