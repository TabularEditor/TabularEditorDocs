...
export async function renderNavbar(): Promise<NavItem[]> {
  ...
  const menuItem = item => {
    ...
    return html`<li class='nav-item'><a class='btn-style--secondary btn-link btn-secondary1 btn-regular ${active}' aria-current=${current} href=${item.href}>${breakWordLit(item.name)}</a></li>`
  }
