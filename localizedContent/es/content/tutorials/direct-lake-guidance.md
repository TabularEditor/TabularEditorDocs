...
La tabla importada llega con sus columnas ya en su sitio, en el mismo orden que en la tabla de origen. Cuando Tabular Editor no puede leer el esquema porque no se pudo determinar el punto de conexión de análisis SQL del Lakehouse o del Warehouse y no se indicó ninguno en la configuración de importación, lo informa y especifica lo que necesita, en lugar de crear una tabla sin columnas. No hace falta ejecutar **Update Table Schema** después de una importación correcta.

## Inspeccionar una partición de Direct Lake

Al seleccionar una partición de Direct Lake en @tom-explorer-view, se muestra su **Expresión de definición de cobertura de datos** en el **Editor de expresiones** cuando hay una definida; de lo contrario, el editor queda vacío. Una partición de Direct Lake no tiene una expresión de consulta propia, así que no hay nada más que mostrar. Lo mismo se aplica a las particiones generadas por una política de actualización incremental.
...
3. **Actualizar metadatos de columnas**: La partición que configuraste manualmente todavía no tiene columnas, así que usa la función **Update Table Schema** de Tabular Editor para obtener los nombres de las columnas y los tipos de datos del Lakehouse/Warehouse. Como alternativa, crea columnas de datos manualmente (**Alt+4**) y especifica `Name`, `Data Type`, `Source Column` y cualquier otra propiedad relevante para cada columna. Este paso solo forma parte del procedimiento manual. Una tabla añadida mediante el Asistente para importar tablas ya tiene sus columnas.
