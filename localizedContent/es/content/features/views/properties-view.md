...
La barra de herramientas en la parte superior de la vista de propiedades contiene los siguientes botones:

- **Por categorías**: Agrupa las propiedades en categorías como _Básico_, _Metadatos_ y _Opciones_.
- **Alfabético**: Muestra todas las propiedades en una única lista ordenada alfabéticamente.
- **Mostrar cambios**: Oculta todas las propiedades que no han cambiado desde la última vez que se guardó el modelo, de modo que solo queden las propiedades con [cambios no guardados](xref:unsaved-changes). Mientras el filtro está activo, el título de la vista muestra **Propiedades (Modificadas)**.
...
Las propiedades que difieren de la última versión guardada del modelo se muestran con un fondo de fila naranja claro. Cuando se seleccionan varios objetos, una fila se marca si alguno de los objetos seleccionados ha cambiado esa propiedad.

<figure style="padding-top: 15px;">
  <img class="noscale" src="~/content/assets/images/unsaved-changes/revert-property.png" alt="Properties view with unsaved changes" style="width: 500px;"/><figcaption style="font-size: 12px; padding-top: 10px; padding-bottom: 15px; padding-left: 75px; padding-right: 75px; color:#00766e"><strong>Figura 2:</strong> Una medida con cambios no guardados en su Descripción y Cadena de formato. La opción <strong>Revertir</strong> restablece una sola propiedad a su valor guardado.</figcaption>
</figure>

Haz clic con el botón derecho en una fila marcada y elige **Revertir** para devolver esa propiedad al valor que tenía en el último guardado, sin tocar ningún otro cambio no guardado. Revertir cuenta como un solo paso en la pila de deshacer, así que **Ctrl+Z** vuelve a aplicar el cambio. Consulta @unsaved-changes para obtener más detalles, incluido cómo revertir objetos completos desde el Explorador TOM y cómo desactivar los indicadores en **Herramientas > Preferencias**.
