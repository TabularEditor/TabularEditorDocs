...
- **PATH de archivos** — dónde lee la CLI las macros y las reglas de BPA, y dónde escribe el registro de consultas.
- **Valores predeterminados de comportamiento** — controles de BPA, formato automático y validación.
- **Perfiles de conexión guardados** — la lista de perfiles con nombre entre los que puedes cambiar.
...
`TE_CONFIG` se respeta de forma coherente en todas las operaciones del archivo de configuración: `te config list`, `te config set`, `te config init` y `te config paths` leen y escriben en el PATH resuelto. Está pensado principalmente para pruebas, instalaciones mediante scripts y configuración por entorno.
...
| ---------------------------- | -------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `autoFormat`                 | `false`        | Da formato automáticamente a las expresiones DAX modificadas por un comando de modificación. El formato se limita a los objetos que el comando haya modificado, pero abarca todas las propiedades de expresiones DAX que contengan (expresiones, expresiones de cadena de formato dinámica, filas de detalle, objetivo/estado/tendencia del KPI, expresiones de grupo de cálculo y de permiso de tabla, etc.). Las consultas de partición de Power Query (M) y SQL nunca se reformatean. Siempre usa el formateador integrado sin conexión en el dialecto de comas; se aplican las claves de diseño de `formatOptions`.                                                                    |
| `validateOnMutation`         | `true`         | Después de un comando de modificación (`add`, `set`, `mv`, `macro run`), comprueba que todas las referencias `Table[Column]` del modelo sigan resolviéndose. Detecta referencias huérfanas introducidas por cambios de nombre o eliminaciones antes de llegar al despliegue.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `mutationOutput`             | `diff`         | Cómo los comandos de mutación (`add`, `set`, `move`, `remove`, `script`, `bpa run --fix`) muestran el conjunto de cambios resultante en la salida de texto: `diff` (diff completo de antes/después), `stat` (recuentos de cambios por objeto), `name-only` (rutas de los objetos modificados) o `none` (suprime el conjunto de cambios; solo configuración: no existe la opción `--none`). Las opciones `--diff` / `--stat` / `--name-only` de cada comando anulan esta configuración para una sola ejecución. La salida JSON siempre incluye el array `changes` completo.                                                        |
...
| `bpa.onDeploy`               | `true`         | Ejecuta el control de BPA antes de que se ejecute `te deploy`. El despliegue se cancela si alguna regla se activa con una gravedad >= error. Omítelo en una invocación concreta con `--skip-bpa`, o corrígelo automáticamente con `--fix-bpa`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `launchInteractiveMode`      | `auto`         | Indica si ejecutar `te` en un terminal sin argumentos inicia el REPL interactivo. `auto` (predeterminado) inicia el REPL solo cuando los tres flujos (stdin, stdout, stderr) están asociados a un TTY, por lo que los scripts y los pipelines de CI pasan al análisis normal. `always` inicia el REPL independientemente de si hay redirección. `never` desactiva por completo el inicio automático y restaura el comportamiento tradicional de mostrar la ayuda cuando no hay entrada. La opción global `--non-interactive` fuerza `never` para una sola ejecución. También se puede establecer para una sola ejecución mediante la variable de entorno `TE_INTERACTIVE`. |
...
Se aplica cada vez que la CLI formatea DAX. La CLI incluye un formateador que funciona completamente sin conexión. Las claves de diseño (`shortFormat`, `skipSpaceAfterFunction`) se aplican cuando `autoFormat` vuelve a dar formato a expresiones modificadas y cuando `te query` representa el texto de la consulta; en cambio, el formato explícito mediante `te set <path> --format <Property>` y `te util format-dax` utiliza las opciones equivalentes por ejecución (`--long`, `--no-space-after-function`). Deliberadamente no hay ninguna clave para el separador de listas: el DAX almacenado en un modelo o enviado a Analysis Services siempre va separado por comas, por lo que todas las operaciones de formato controladas por la configuración usan comas. El único caso en que se aplica el dialecto de punto y coma es la opción `--semicolons` de `te util format-dax`, para el DAX que hayas escrito tú mismo con punto y coma. `formatOptions.useSqlBiDaxFormatter` redirige el formato explícito y la representación de `te query` a través del servicio web SQL BI [daxformatter.com](https://www.daxformatter.com) (requiere acceso a Internet) si necesitas ese estilo; `autoFormat` siempre usa el formateador integrado.
...
`te profile set` expone indicadores para los más habituales (`--auto-format`, `--validate-on-mutation`, `--bpa-on-mutation`, `--bpa-on-deploy`, `--vertipaq-on-refresh`, `--spinner`); cada uno acepta `true`, `false` o `null` para borrar la sobrescritura.
...
- `te save-as` ejecuta la validación, a menos que se pase `--skip-bpa` (o `--force`) o que `bpa.onSave` sea `false`.
- `te add`, `te set`, `te move`, `te remove`, `te macro run` ejecutan la validación solo cuando `bpa.onMutation` es `true`.
...
Cuando se ejecuta la validación y se encuentran violaciones con severidad >= `error`, el comando falla con el código de salida `1` y un resumen de las violaciones. Opciones para resolverlo:
...
- `--bpa-rules <path>` - repetible; sobrescribe `bpa.rules` para esta única invocación de `te deploy` o `te save-as`. Las reglas integradas siguen aplicándose salvo que `bpa.builtInRules` sea `false`.
...
Cuando ejecutas un comando que realiza cambios (`te add`, `te set`, `te move`, `te macro run`), la CLI realiza estas comprobaciones automáticamente:
...
En Windows, `te` respeta las mismas políticas de administrador que Tabular Editor 3. Las políticas se leen del Registro en `Software\Policies\Tabular Editor ApS`, con una subclave opcional `TECLI` para los valores que solo deben aplicarse a la CLI y una subclave `TE3` para la aplicación de escritorio; también se leen de la clave anterior `Software\Policies\Kapacity\Tabular Editor`, que sigue funcionando sin cambios. Un valor de todo el equipo (`HKEY_LOCAL_MACHINE`) tiene prioridad sobre uno por usuario (`HKEY_CURRENT_USER`), y dentro de una colmena, un valor específico del producto tiene prioridad sobre uno compartido. Cuando una política desactiva una funcionalidad, el comando indica qué política es la responsable, no hace nada y termina con error, de modo que una canalización que dependa de algo que un administrador haya desactivado desde entonces falle de forma visible en lugar de Reportar éxito por un trabajo que nunca hizo.

| Política               | Efecto en la CLI                                                                                                                                                                                                                                                                                                                                                |
| ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DisableCSharpScripts` | Rechaza `te script` y las correcciones automáticas de `te bpa run --fix`.                                                                                                                                                                                                                                                                       |
| `DisableMacros`        | Rechaza todos los comandos `te macro`.                                                                                                                                                                                                                                                                                                          |
| `DisableBpaDownload`   | Rechaza las reglas de Best Practice Analyzer proporcionadas como una URL. Los archivos de reglas en disco y las reglas integradas no se ven afectados.                                                                                                                                                                          |
| `DisableTelemetry`     | Desactiva las estadísticas anónimas de uso, independientemente de lo que indique `disableTelemetry` en la configuración.                                                                                                                                                                                                                        |
| `BlockUnsafeScripts`   | Permite `te script`, `te macro run` y `te bpa run --fix` solo cuando el código permanece dentro del modelo semántico. Cualquier acción que lea o escriba archivos, acceda a la red, inicie otro programa o haga referencia a un ensamblado externo se rechaza antes de ejecutarse; en la salida JSON aparece `blockedByPolicy`. |

Las directivas que rigen características que la CLI no tiene —comprobaciones de actualización, Reports de errores, el Optimizador de DAX, el Administrador de paquetes de DAX, el asistente de IA y el servidor MCP— no tienen ningún efecto en la CLI. Consulta @policies para ver la lista completa de directivas y cómo implementarlas.

`BlockUnsafeScripts` requiere la Edición Enterprise de Tabular Editor 3 en la aplicación de escritorio. La CLI no tiene ediciones, así que simplemente aplica la directiva allí donde la encuentre. Consulta [C# Scripts](xref:csharp-scripts#administrator-policies) para saber qué se considera mantenerse dentro del modelo.
...
| `TE_SESSION`     | Sobrescribe el identificador de sesión por terminal que se usa para el estado de la conexión activa. Útil para ejecutar varias sesiones aisladas de la CLI dentro del mismo shell, por ejemplo, en trabajos de matriz de CI en paralelo. Inspeccione y administre sesiones con [`te session`](xref:te-cli-commands#session).                                                                                           |
| `TE_INTERACTIVE` | Permite anular `launchInteractiveMode` para una sola invocación. Acepta `auto`, `always` o `never`. Útil para scripts de una sola vez que necesiten el REPL interactivo (`TE_INTERACTIVE=always`) o que quieran forzar el comportamiento clásico de mostrar la ayuda cuando no se proporciona ningún comando (`TE_INTERACTIVE=never`) sin modificar el archivo de configuración. |
