...
| `BlockUnsafeScripts`   | Permite `te script`, `te macro run` y `te bpa run --fix` únicamente cuando el código se mantiene dentro del modelo semántico. Cualquier acción que lea o escriba en un archivo, acceda a la red, inicie otro programa o haga referencia a un ensamblado externo se rechaza antes de ejecutarse, con `blockedByPolicy` en la salida JSON. |
...
`BlockUnsafeScripts` requiere la Edición Enterprise de Tabular Editor 3 en la aplicación de escritorio. La CLI no tiene ediciones, así que simplemente aplica la directiva donde la encuentre. Consulta [C# Script](xref:csharp-scripts#administrator-policies) para ver qué se considera que permanece dentro del modelo.
