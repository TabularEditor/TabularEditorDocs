...
## Pilares de diseño y públicos objetivo

Cuatro pilares de diseño guían todos los comandos:
...
- **Seguro de forma predeterminada** - los comandos de edición, como `te set`, `te add` y `te remove`, muestran el cambio en forma de diff y no escriben nada hasta que agregues `--save`; si agregas `--force`, se guarda incluso cuando el cambio introduce errores de validación. `te deploy` y `te refresh` muestran el TMSL exacto que enviarían hasta que agregues `--execute`, y después piden confirmación, a menos que agregues `--force`.
...
> [!Note]
> Cuando uses la TE CLI con agentes, utiliza la [skill del TE CLI para agentes de codificación con IA](https://github.com/TabularEditor/CLI/tree/main/skills/te-cli), que encapsula la TE CLI de principio a fin. Consulta [AI Agent Skill](xref:te-cli-skill) para saber cómo instalarla.
...
| [Implementación y actualización](xref:te-cli-commands#deployment-and-refresh)              | Implementar en el Workspace, desencadenar una actualización y aplicar políticas de actualización | [`te deploy`](xref:te-cli-commands#deploy), [`te refresh`](xref:te-cli-commands#refresh)                                                                                                  |
...
| [Shell](xref:te-cli-commands#shell)                                                        | Modo interactivo, estado de la sesión y autocompletado del shell                                 | [`te interactive`](xref:te-cli-commands#interactive), [`te session`](xref:te-cli-commands#session), [`te completion`](xref:te-cli-commands#completion)                                    |

> [!TIP]
> La documentación usa verbos canónicos en su forma larga (`list`, `remove`, `move`), pero las formas cortas clásicas siguen funcionando como alias (`ls`, `rm`, `mv`, `rename`). Esto se aplica a los comandos de nivel superior y a los subcomandos `remove` / `list` dentro de grupos como `te bpa rules`, `te macro`, `te config`, `te profile`, `te session` y `te test`. Consulta @te-cli-commands#command-aliases para ver la correspondencia completa.
...
4. **Ejecuta tu primer comando**: `te --help` enumera todos los comandos; `te <command> --help` muestra las opciones detalladas. Sugerencia: si ejecutas `te` sin más en una terminal, accederás a la REPL interactiva, una forma práctica de explorar un modelo. Consulta @te-cli-interactive.
...
> [!WARNING]
> El banner vuelve a aparecer con cada comando durante los **14 días anteriores a la fecha de finalización de la versión preliminar** (2026-10-31), independientemente de `hidePreviewNotice`. Esto garantiza que veas una advertencia antes de que la CLI deje de funcionar.
