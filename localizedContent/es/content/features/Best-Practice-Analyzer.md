...
- **Reglas para el usuario local**: Son reglas que se almacenan en su archivo `%AppData%\\..\\Local\\TabularEditor\\BPARules.json`. Estas reglas se aplicarán a todos los modelos que el usuario de Windows que haya iniciado sesión cargue en Tabular Editor.
