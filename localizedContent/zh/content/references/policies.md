...
某些 IT 组织可能希望限制 Tabular Editor 的部分功能。 可以通过组策略实现：在 Windows 注册表中设置特定值。 可以通过组策略实现：在 Windows 注册表中设置特定值。
...
| 值                              | 启用后……                                                                                                                                                      |
| ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| DisableUpdates                 | Tabular Editor 不会在线检查是否有新版本可用。 此外，用户也无法通过该工具手动检查是否有更新。 此外，用户也无法通过该工具手动检查是否有更新。                                                                             |
| DisableCSharpScripts           | Tabular Editor 不允许用户创建并执行 C# Script。                                                                                                                       |
| DisableMacros                  | Tabular Editor 不允许用户保存或运行宏。 此外，定义在 %LocalAppData% 文件夹中的宏也不会在应用程序启动时加载并编译。 此外，定义在 %LocalAppData% 文件夹中的宏也不会在应用程序启动时加载并编译。                                    |
| DisableBpaDownload             | Tabular Editor 不允许从网络下载 Best Practice Analyzer 规则。                                                                                                         |
| DisableWebDaxFormatter         | Tabular Editor 将禁用 DAX 代码格式化程序，该程序会向 daxformatter.com 发起 Web 请求。 （TE3 仍允许通过内置的 DAX 格式化程序来格式化代码） （TE3 仍允许通过内置的 DAX 格式化程序来格式化代码）             |
...
| RequireDaxOptimizerObfuscation | **（仅限 TE3）** 用户将无法通过 DAX优化器集成功能上传 VPAX 文件。 当强制执行 `DisableDaxOptimizer` 时，会隐式强制执行此项。 当强制执行 `DisableDaxOptimizer` 或 `DisableDaxOptimizerUpload` 时，会隐式强制执行此项。 |
...
如果希望禁用所有 AI 功能，请设置 `DisableAi` 策略。 如果希望禁用所有 AI 功能，请设置 `DisableAi` 策略。 这会阻止 AI 模块在启动时加载，并清除任何已存储的 API 密钥配置。