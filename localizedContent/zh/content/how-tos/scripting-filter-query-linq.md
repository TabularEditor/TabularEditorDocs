...
C# Script 使用标准 LINQ 方法来筛选、搜索和转换 TOM 对象集合。 These patterns are building blocks. Use collection-returning methods in `foreach` loops, bool-returning methods in `if` conditions and scalar-returning methods in variable assignments.
...
`Where()` 返回所有满足谓词条件的对象。 Chain multiple conditions with `&&` and `||`.
...
> [!WARNING]
> 使用 `Contains()` 进行字符串匹配时，会在表达式中的任意位置查找文本，包括字符串字面量和注释内部。 To detect actual DAX function usage, analyze the tokenized expression instead.
...
`First()` returns the first match or throws if none exist. `FirstOrDefault()` 会返回 null，而不是引发异常。
...
`Select()` 会转换每个元素。 Use it to extract property values or build new structures.
...
When you modify objects inside a loop (delete, add, move), you change the collection being iterated. 一定要先调用 `.ToList()` 或 `.ToArray()` 来创建快照。
...
在 BPA 规则表达式中，语法与 C# LINQ 不同。 Dynamic LINQ has no lambda arrows, uses keyword operators and compares enums as strings.
...
> [!NOTE]
> 动态 LINQ 表达式会针对上下文中的单个对象进行求值。 There is no equivalent to `Model.AllMeasures` or cross-table queries. Each BPA rule runs its expression once per object in its scope.
