...
透视用于控制哪些对象会显示在特定的客户端视图中。 翻译（区域设置）提供本地化的名称、描述和显示文件夹。 两者都使用 TOM 对象上的索引器属性。 透视用于控制哪些对象会显示在特定的客户端视图中。 翻译（区域设置）提供本地化的名称、描述和显示文件夹。 两者都使用 TOM 对象上的索引器属性。 有关如何访问 TOM 对象及其索引器的详细信息，请参阅 @how-to-navigate-tom-hierarchy。
