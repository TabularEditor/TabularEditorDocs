...
> [!WARNING]
> 兼容级别升级是单向的。 你可以升级，但无法可靠地降级。 将其视为一次架构升级，并先验证你的部署目标。 不能。 不支持降级，而且这也不是安全或可靠的补救措施。 将其视为一次架构升级，并先验证你的部署目标。
...
不同平台（SSAS、Azure Analysis Services、Fabric/Power BI Premium）对兼容级别的支持情况不同。 如果你的部署目标不支持所选级别，将无法部署。 不同平台（SSAS、Azure Analysis Services、Fabric/Power BI Premium）对兼容级别的支持情况不同。 如果你的部署目标不支持所选级别，将无法部署。 参阅 [Analysis Services 中表格模型的兼容级别](https://learn.microsoft.com/en-us/analysis-services/tabular-models/compatibility-level-for-tabular-models-in-analysis-services)
