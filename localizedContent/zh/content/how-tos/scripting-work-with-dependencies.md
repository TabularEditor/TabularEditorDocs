...
`ReferencedBy` 适用于任何 (xref:TabularEditor.TOMWrapper.IDaxObject) 对象。 `ReferencedBy` 适用于任何 (xref:TabularEditor.TOMWrapper.IDaxObject) 对象。 这也包括自身不含 DAX 表达式的对象，例如 `DataColumn`；它们仍可在其他对象的 DAX 中通过名称被引用。
