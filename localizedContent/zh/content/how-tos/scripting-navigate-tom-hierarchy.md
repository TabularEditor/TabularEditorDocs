...
在 Best Practice Analyzer (BPA) 规则表达式和 **TOM Explorer** 树筛选器中，可直接访问当前上下文对象上的属性。 父级导航使用点表示法。 父级导航使用点表示法。
...
> [!NOTE]
> BPA 规则中的 Dynamic LINQ 表达式每次只会针对单个对象求值。 你无法访问 `Model` 或跨表的集合。 使用规则的 **适用于** 作用域，选择该表达式要针对哪种对象类型运行。 你无法访问 `Model` 或跨表的集合。 使用规则的 **适用于** 作用域，选择该表达式要针对哪种对象类型运行。
