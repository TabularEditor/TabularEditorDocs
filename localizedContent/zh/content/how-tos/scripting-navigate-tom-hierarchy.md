...
每个 C# Script 都以 `Model` 对象或 @csharp-scripts 的 `Selected` 对象为起点。 These expose the Tabular Editor TOM wrapper, which wraps the Microsoft Analysis Services Tabular Object Model (TOM). See the (xref:TabularEditor.TOMWrapper) API reference for the full wrapper documentation.
...
在任何集合中使用索引器 `["name"]`，即可按精确名称检索对象。 This throws an exception if the name does not exist.
...
计算组表是 `Table` 的一个子类型。 Access them through `Model.CalculationGroups` and iterate their items.
...
关系定义在 `Model` 上，而不是在表上。 Each relationship references its from/to columns and tables.
...
在 Best Practice Analyzer (BPA) 规则表达式和 **TOM Explorer** 树筛选器中，可直接访问当前上下文对象上的属性。 Parent navigation uses dot notation.
...
> [!NOTE]
> BPA 规则中的 Dynamic LINQ 表达式每次只会针对单个对象求值。 You do not have access to `Model` or cross-table collections. Use the rule's **Applies to** scope to select which object type the expression runs against.
