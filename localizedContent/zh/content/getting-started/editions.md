...
> [!NOTE]
> Tabular Editor 3 的许可证为**按开发者授权**。 换句话说，只有实际使用 Tabular Editor 3 产品的人才需要许可证。 换句话说，只有实际使用 Tabular Editor 3 产品的人才需要许可证。

## 支持的 Data model 建模场景

Tabular Editor 3 各版本的主要区别在于它们支持哪些类型的表格 Data model 建模场景。 要理解这一差异，可以把 Analysis Services（Tabular）看作有多种不同的“形态”： 要理解这一差异，可以把 Analysis Services（Tabular）看作有多种不同的“形态”：
...
> [!NOTE]
> 请注意，Power BI Desktop [目前并不支持所有 Data model 建模操作](xref:desktop-limitations)。 因此，Tabular Editor 3 默认会阻止 Power BI Desktop 不支持的操作。 不过，你可以在“工具 > 偏好 > Power BI”中解除该限制。 因此，Tabular Editor 3 默认会阻止 Power BI Desktop 不支持的操作。 不过，你可以在“工具 > 偏好 > Power BI”中解除该限制。
...
你可以随时在工具中停用现有安装：在“帮助 > 关于 Tabular Editor”中选择“更改许可证密钥...”选项。 你也可以通过我们的[自助门户](https://tabulareditor.com/sign-in)停用安装：进入“Licenses”选项卡。 你也可以通过我们的[自助门户](https://tabulareditor.com/sign-in)停用安装：进入“Licenses”选项卡。
