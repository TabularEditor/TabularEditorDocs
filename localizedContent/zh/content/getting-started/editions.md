...
> [!NOTE]
> Tabular Editor 3 licenses are **per developer**. 换句话说，只有实际使用 Tabular Editor 3 产品的人才需要许可证。

## 支持的 Data model 建模场景

The main difference between the various editions of Tabular Editor 3, is which types of tabular data modeling scenarios they support. 要理解这一差异，可以把 Analysis Services（Tabular）看作有多种不同的“形态”：
...
> [!IMPORTANT]
> Tabular Editor only allows editing data models using Compatibility Level 1200 or higher. 自 SQL Server 2016 起，Analysis Services 的所有实例默认都是如此。 出于同样的原因，Tabular Editor 不支持 Excel PowerPivot，因为它使用更早的兼容级别。
...
> [!NOTE]
> Please keep in mind that Power BI Desktop [currently does not support all Data modeling operations](xref:desktop-limitations). 因此，Tabular Editor 3 默认会阻止 Power BI Desktop 不支持的操作。 不过，你可以在“工具 > 偏好 > Power BI”中解除该限制。

> [!IMPORTANT]
> Tabular Editor can only be used as an external tool for Power BI Desktop when the Power BI report (.pbix, .pbip or .pbit) file contains a data model (Import, DirectQuery or Composite). **不支持使用 Live connection 的 Report**，因为这些 Report 不包含 Data model。 [更多信息](xref:desktop-limitations)。
...
You can deactivate an existing installation at any time from within the tool itself, by choosing the "Change license key..." option under "Help > About Tabular Editor". 你也可以通过我们的[自助门户](https://tabulareditor.com/sign-in)停用安装：进入“Licenses”选项卡。
