...
> [!NOTE]
> Tabular Editor 现已提供跨平台 CLI（`te`），目前处于有限公开预览阶段，专为 CI/CD 场景打造——支持非交互模式、原生 GitHub Actions/Azure DevOps 注解、VSTEST 输出，以及用于在管道中运行回归测试的 `te test run` 命令。 它与下文描述的这类管道天然契合，值得关注。 As of this writing, Tabular Editor's own documentation advises against using it in production pipelines during preview (the preview build is stated to expire 2026-10-31), so the reference implementation in this article uses the established `TabularEditor.exe` CLI instead. 有关这个新 CLI 当前具备的能力和示例，可以查看 [CI/CD 集成](xref:te-cli-cicd)。
