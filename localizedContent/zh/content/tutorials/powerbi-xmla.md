...
你可以使用 Tabular Editor 3，通过 [XMLA endpoint](https://docs.microsoft.com/en-us/power-bi/admin/service-premium-connect-tools) 连接到发布到 Power BI 服务的 Power BI Dataset。 XMLA endpoint 可用于 Power BI Premium 容量 Workspace（即分配到 Px、Ax 或 EMx SKU 的 Workspace），或 Power BI Premium Per User（PPU）Workspace。 XMLA endpoint 可用于 Power BI Premium 容量 Workspace（即分配到 Px、Ax 或 EMx SKU 的 Workspace），或 Power BI Premium Per User（PPU）Workspace。

> [!NOTE]
> 仅有 Power BI Pro 许可证不足以在共享 Workspace 中访问 Power BI Dataset。 进行 XMLA 访问需要 Premium/Embedded 容量或 Power BI Premium Per User 许可。 进行 XMLA 访问需要 Premium/Embedded 容量或 Power BI Premium Per User 许可。

## 先决条件

Tabular Editor 要求 XMLA endpoint 同时允许读取和写入访问。 该设置由容量管理员控制，详见[这里](https://docs.microsoft.com/en-us/power-bi/admin/service-premium-connect-tools#enable-xmla-read-write)。 该设置由容量管理员控制，详见[这里](https://docs.microsoft.com/en-us/power-bi/admin/service-premium-connect-tools#enable-xmla-read-write)。
