...
在 **Tabular Editor 3 (TE3)** 中刷新模型时可能会出现这个错误，即使受影响的表并未直接引用 `CALENDAR()` 函数。 这通常表示某个依赖的日期表或日历表依赖其他表中的值，而这些表在刷新过程中会暂时为空，导致开始日期或结束日期的值为空白。 这通常表示某个依赖的日期表或日历表依赖其他表中的值，而这些表在刷新过程中会暂时为空，导致开始日期或结束日期的值为空白。
...
2. **检查空表**
   - 确认所有被引用的表都包含数据。 确认所有被引用的表都包含数据。 如果源表为空，请刷新数据源或调整架构变量配置。
3. **添加默认兜底值**
