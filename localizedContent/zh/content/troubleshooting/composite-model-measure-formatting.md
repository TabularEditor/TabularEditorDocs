...
复合模型通过实时连接，将本地 Power BI 表与 SSAS/AAS 语义模型中的远程表组合在一起。在这种架构下，度量值的格式设置可能存在歧义：
...
1. **所有权不明确**：远程度量值由远程 SSAS/AAS 模型拥有。当你在 Tabular Editor 中编辑格式时，可能是在尝试覆盖远程元数据，这会造成冲突。

2. **元数据同步**：当度量值上存在格式字符串表达式时，FormatString 通常会显示为“自定义”，用来表示已启用动态格式。如果随后又尝试同时设置静态的 FormatString，这两个属性都会被赋值，从而触发验证错误。

3. **持久化限制**：对远程度量值元数据的更改可能无法可靠地保存，因为远程模型保留最终控制权。这会使本地复合模型处于不一致状态。
...
- 计算组可以定义格式字符串表达式，从而覆盖度量值格式。如果某个计算项的格式表达式处于生效状态，UI 可能仍会显示度量值的静态 FormatString，从而看起来像是两者都已设置。
...
- [ ] **检查格式字符串表达式**：即使你没设置，也要确认它是否已被填充。在属性网格中，查找不为空的“格式字符串表达式”字段。
...
2. **审查远程度量值**：在复合模型中编辑格式之前，先确认该度量值是否为远程度量值。如果是，就在源 SSAS/AAS 模型中管理格式设置。
