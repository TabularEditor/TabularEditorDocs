...
此最佳实践规则会识别可见的数值列（Int64、Decimal、Double），其默认聚合行为（`SummarizeBy`）不为 `None`。 大多数数值列不应被自动聚合，因为对 ID、非可加语境下的数量或代码等数值求和会产生毫无意义的结果。 大多数数值列不应被自动聚合，因为对 ID、非可加语境下的数量或代码等数值求和会产生毫无意义的结果。
