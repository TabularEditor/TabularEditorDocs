...
此最佳实践规则会识别那些将 `IsAvailableInMDX` 设为 `false`，但实际用于需要 MDX 访问场景的列。 这些列必须启用 MDX 可用性，才能在层次结构、关系和排序操作中正常工作。 这些列必须启用 MDX 可用性，才能在层次结构、关系和排序操作中正常工作。
...
当某列用于特定的模型结构时，Analysis Services 引擎需要通过 MDX 访问该列。 对需要 MDX 的列禁用 MDX 访问会导致： 对需要 MDX 的列禁用 MDX 访问会导致：
