...
宏是在 Tabular Editor 中保存的 C# Script，可在不同语义模型间轻松复用。将脚本保存为宏后，你就可以在 TOM Explorer 中右键单击对象时使用该宏，从而轻松将脚本应用到你的模型中。
...
> [!TIP]
> 开始使用 C# 脚本编写的一个简单方法是使用内置的录制功能，它可以录制你在 TOM Explorer 中执行的操作。通过这种方式，你可以了解如何与不同的模型对象交互，并创建可复用的脚本。另一种方法是复用现有脚本，例如我们[脚本库](xref:csharp-script-library)中的脚本。在本教程中，我们使用脚本 [Format Numeric Measures](xref:script-format-numeric-measures) 来演示宏功能。
