...
To use the script, select the date column in the table for which you want to configure incremental refresh, then run the script. 仅当你尚未创建 #"RangeStart" 和 #"RangeEnd" 参数，并且所选表尚未配置刷新策略时，脚本才会运行。
...
<figure style="padding-top: 15px;">
  <img class="noscale" src="~/content/assets/images/Cscripts/script-configure-incremental-refresh.png" alt="The prompt that helps you configure incremental refresh based on selected columns" style="width: 550px;"/><figcaption style="font-size: 12px; padding-top: 10px; padding-bottom: 15px; padding-left: 75px; padding-right: 75px; color:#00766e"><strong>图 1：</strong>运行脚本时，系统会提示你选择要配置的表，以及要为其配置策略的 DateTime 或 Int 列。 Then, the dialog in this image will appear to let you enter the Refresh Policy parameters.</figcaption>
</figure>
