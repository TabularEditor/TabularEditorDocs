...
These are more basic scripts that are easy to understand or modify. They have a defined scope and limited complexity; you don't need a reasonable knowledge of the C# language to use, understand and modify these scripts. 因此，在 Tabular Editor 中开始编写 C# Script 时，这是一个很好的起点。
...
| [编辑隐藏分区](xref:script-edit-hidden-partitions)              | 在 Calc 中显示隐藏分区的属性。 Groups & Calc. 表                                       | 当你需要查看或编辑这些隐藏分区的 TOM 属性时。                                                                                                                    |
| [格式化数值度量值](xref:script-format-numeric-measures)           | 格式化所选度量值                                                                                                      | 当你想快速将格式字符串应用到当前选中的度量值时                                                                                                                      |
| [显示数据源依赖关系](xref:script-show-data-source-dependencies)    | 显示数据源的依赖关系                                                                                                    | For explicit (legacy) data sources it can be hard to know exactly where they are used. 此脚本会显示哪些分区引用了所选数据源 |
