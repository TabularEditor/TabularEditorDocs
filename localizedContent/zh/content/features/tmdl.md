...
要在 Tabular Editor 3 中启用 TMDL，请前往 **工具 > 偏好 > 文件格式 > 保存到文件夹**，然后在 **序列化模式** 下拉列表中选择“TMDL”。 The legacy "save-to-folder" functionality will continue to exist side by side with TMDL, but is not a Microsoft supported format.

完成后，当你将模型保存为文件夹（**文件 > 保存到文件夹...**）时，Tabular Editor 3 将使用 TMDL 格式。

> [!NOTE]
> 当你从旧版 Tabular Editor 文件夹结构加载模型时，使用 **文件 > 保存**（Ctrl+S）仍将以相同的格式保存。 Only when you explicitly use the **File > Save to folder...** command, will the model be saved in the new TMDL format.
...
TMDL is fully compatible with Microsoft Fabric's Git integration feature. 在 Tabular Editor 3 中使用 **保存并包含支持文件** 选项时，TMDL 序列化格式会创建一个文件夹结构，其中包含 Fabric 的 Git 集成所需的全部元数据文件。
...
此组合使你能够将语义模型提交到 Git repository，并使用 Fabric 内置的 Git 集成功能将其与 Microsoft Fabric Workspace 同步。 The human-readable nature of TMDL makes it particularly well-suited for code reviews and tracking changes in version control systems.
