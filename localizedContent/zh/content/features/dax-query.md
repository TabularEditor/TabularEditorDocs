...
5. **自动执行查询**：用于跟踪已连接的语义模型，并在模型发生变化时自动更新查询结果。 例如，修改某个度量值时，它可以帮助你了解结果会如何变化。 例如，修改某个度量值时，它可以帮助你了解结果会如何变化。
6. **保留排序和筛选**：用于控制在执行查询时，结果网格（一个或多个）中排序和筛选状态的保留方式。 提供三种偏好： 提供三种偏好：
   - **从不**：每次运行查询时都会重置排序和筛选。
...
从 Tabular Editor 3.23.0 起，“应用”和“应用选择”也会处理 DEFINE COLUMN 和 DEFINE TABLE 语句。 从 Tabular Editor 3.23.0 起，“应用”和“应用选择”也会处理 DEFINE COLUMN 和 DEFINE TABLE 语句。 Tabular Editor 会在模型中创建相应的计算列/计算表；如果它们已存在，则会更新其表达式。
...
“应用”选项会将查询中通过 DEFINE 显式定义的所有度量值、列或表的 DAX 表达式同步到相应对象的定义中。 任何尚不存在的度量值、列或表都会被创建。 任何尚不存在的度量值、列或表都会被创建。
...
在同一个 DAX 查询中包含多个 EVALUATE 语句完全可行。 这种查询类型最常见于 Power BI 性能分析器的查询。 这种查询类型最常见于 Power BI 性能分析器的查询。
...
Tabular Editor 3 从 3.16.0 版本开始，新增将 DAX 查询结果导出为 CSV 或 Excel 的功能。 运行 DAX 查询后，工具栏上会激活一个按钮，让你可以将结果以 CSV 或 Excel 格式保存到本地。 运行 DAX 查询后，工具栏上会激活一个按钮，让你可以将结果以 CSV 或 Excel 格式保存到本地。
