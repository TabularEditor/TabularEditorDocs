...
Semantic Bridge 包含一个对象模型，用于表示 Databricks Metric View。
...
> [!NOTE]
> 在对象模型中，我们遵循 C# 命名约定，因此对象模型中的所有类型名和属性名均使用 `PascalCase`。
...
为评估验证规则，会先检查 `View`，然后按顺序验证各个集合：先是 `Joins`，再是 `Dimensions`，最后是 `Measures`。
