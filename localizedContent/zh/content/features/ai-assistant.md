...
选择 **Anthropic** 作为提供商，然后输入你的 API 密钥。 默认模型是 **claude-sonnet-4-6**。 You can change the model name to any Anthropic model available on your account.
...
3. Start the LM Studio server. 可以通过左侧面板的开发者页面或 CLI 来完成。 例如：`lms server start`
...
> [!TIP]
> We recommend a model with a _minimum_ of 30 billion parameters but ideally at least 100 billion parameters. 例如，Qwen3.5-122B-A10B 模型在我们的内部测试中表现良好。
...
- **C# script generation**: Create C# scripts for model modifications that open in a new editor window. 当你在聊天中点击 **Execute** 时，默认会显示 [预览更改](xref:csharp-scripts#running-scripts-with-preview) 对话框，让你在接受前先审阅所有模型元数据更改。 你也可以在编辑器中打开脚本，并从脚本工具栏运行它，可选择是否启用预览。 模型元数据更改可通过 **Ctrl+Z** 撤销
...
The AI Assistant supports multiple simultaneous conversations. 每个对话都会维护各自的信息历史记录和上下文。

- 对话会跨会话保留，并存储在本地的 `%LocalAppData%\TabularEditor3\AI\Conversations\` 中
- 标题会在首次交流后自动生成。 You can rename conversations manually
...
When you execute a C# script from the chat, the **Script Preview** dialog shows a side-by-side diff of all model metadata changes made by the script. 你可以接受这些更改，或将其撤销。 详见[使用预览运行脚本](xref:csharp-scripts#run-c-scripts-with-preview)。

![脚本预览 - 模型更改](~/content/assets/images/preview-script-changes.png)

## 自定义指令

Custom Instructions are instruction sets that guide the AI Assistant's behavior for specific tasks. 它们会根据意图识别自动启用，也可以手动调用。
...
You can create custom instructions by placing `.md` files in `%LocalAppData%\TabularEditor3\AI\CustomInstructions\`. 每个文件都需要包含 YAML front matter，用于定义指令的元数据：
...
The AI Assistant requests permission before sending data to the AI provider. 同意按特定数据类型进行限定：
...
When the AI Assistant needs access to a data type for the first time, a consent dialog appears. 你可以选择同意的持续时间：
...
You can review and reset your consent choices under **Tools > Preferences > AI Assistant > AI Consents**. 每个同意类别都会显示其当前状态。 点击 **Reset** 可撤销处于“Always allowed”状态的同意，并将其恢复为“Ask when needed”。
...
- Requires a user-provided API key. 不包含内置 API 密钥
...
- The AI Assistant cannot connect to a different model from within the chat. 使用 Tabular Editor 的用户界面更改模型连接
- AI 助手无法管理偏好
