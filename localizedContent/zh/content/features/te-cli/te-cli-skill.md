...
- **项目范围** - 技能仅在某个项目或 repository 中可用。 Use this when not every project touches semantic models.
- **用户范围** - 技能在你这台机器上的所有项目中都可用。 Use this if you work with semantic models across many repos.

## 为 Claude Code 安装

Claude Code loads skills from a named folder under `.claude/skills/`. `description` 字段会和你的提示词匹配，所以技能只会在相关时才加载；当你处理无关代码时，它不会消耗任何 token。
...
> [!NOTE]
> Claude Code 会监视技能目录，并在当前会话中识别新增或已编辑的技能，无需重启。 The exception is creating a `.claude/skills/` directory that did not exist when the session started: restart Claude Code once so it begins watching the new directory.
...
You should see `te-cli` in the list. 如果没有，请确认文件路径无误，并且文件以 `---` 开头、第二行包含 `name: te-cli`，然后重启 Claude Code。
...
Claude.ai (web and desktop) has a built-in **Skills** feature. Skills 需要启用代码执行，并且上传时应使用技能文件夹打包成的 ZIP，而不是单独的 `SKILL.md` 文件。

1. 启用代码执行：前往 **Settings > Capabilities**，并开启 **Code execution and file creation**。 On Team and Enterprise plans, an owner enables this in organization settings.
...
VS Code 中的 GitHub Copilot 原生支持 Agent Skills 开放标准——也就是 Claude Code 和 Codex 使用的同一种 `SKILL.md` 格式。 This is the recommended approach because the skill loads only when relevant. For Copilot setups that predate Agent Skills, fall back to the generic `AGENTS.md` install below.
...
Codex CLI loads skills natively from a named folder under `.agents/skills/`, the same directory-based model as Claude Code. 请保留 YAML frontmatter——Codex 要求包含 `name` 和 `description` 字段，并会根据 `description` 来决定何时加载该技能。
