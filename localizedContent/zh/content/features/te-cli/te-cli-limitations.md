...
| 限制                                                                          | 说明 / 变通方法                                                                                                                                                                                                                  |
| --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **未加载 `System.Windows.Forms`**                                              | CLI 使用的是跨平台 `TOMWrapper` 版本，其中剥离了所有与 WinForms 耦合的代码；WinForms 程序集不会被加载到 AppDomain 中。引用 `System.Windows.Forms` 类型（`MessageBox`、`Form`、文件选择器、自定义对话框等）的脚本将无法编译。将所有 UI 交互重构为通过环境变量或 stdin 提供输入。                                 |
