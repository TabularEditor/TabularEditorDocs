...
Tabular Editor CLI (`te`) 是适用于 Power BI 和 Analysis Services 语义模型的跨平台命令行工具。 它以单个自包含的可执行文件形式在 Windows、macOS 和 Linux 上运行，并基于驱动 Tabular Editor 3 的同一基础构建。 它以单个自包含的可执行文件形式在 Windows、macOS 和 Linux 上运行，并基于驱动 Tabular Editor 3 的同一基础构建。
...
与仅限 Windows 的 `TabularEditor.exe` 命令行选项（TE2）不同——后者主要用于从桌面端二进制文件自动执行 C# Script 和宏——`te` 是专为跨平台打造的 CLI，提供结构化输出、可预测的退出代码以及交互式 shell。 这让现有的 [TE2 CLI](xref:command-line-options) 难以很好覆盖的场景成为可能：在 macOS 和 Linux 上通过终端完成模型工作、由 AI 代理直接驱动模型更改，以及无缝接入任何现代 CI 运行器。 这让现有的 [TE2 CLI](xref:command-line-options) 难以很好覆盖的场景成为可能：在 macOS 和 Linux 上通过终端完成模型工作、由 AI 代理直接驱动模型更改，以及无缝接入任何现代 CI 运行器。
...
有关每个命令的语法、选项和示例的完整命令参考，请参阅 @te-cli-commands。 点击表中的任意示例命令，直接跳转到对应的参考条目。 点击表中的任意示例命令，直接跳转到对应的参考条目。
...
3. **进行身份验证**：运行 `te auth login`，即可连接到 Power BI 或 Fabric。 见 @te-cli-auth。 见 @te-cli-auth。
...
> [!WARNING]
> 在预览结束日期（2026-09-30）前 14 天内，无论 `hidePreviewNotice` 如何设置，每次执行命令时该横幅都会再次出现。 这可确保在 CLI 停止运行之前，你能提前看到醒目的警告。 这可确保在 CLI 停止运行之前，你能提前看到醒目的警告。

## 许可概览

在有限公开预览期间，CLI 无需许可证；你只需要一个 Tabular Editor 账户即可下载。 在正式发布 (GA) 时，CLI 将需要许可证；定价仍在最终敲定中，并会在 GA 前公布。 在正式发布 (GA) 时，CLI 将需要许可证；定价仍在最终敲定中，并会在 GA 前公布。
