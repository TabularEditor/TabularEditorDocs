...
| DI014 | [Replace IFERROR with DIVIDE](xref:DI014) | Use the [`DIVIDE`](https://dax.guide/DIVIDE) function instead of [`IFERROR`](https://dax.guide/IFERROR) to provide an alternate result when a division has a zero demoninator. Example:<br>`IFERROR(x / y, 0)` -> `DIVIDE(x, y, 0)` |
| DI015 | Replace IF with DIVIDE | Use the DIVIDE function instead of IF, to more easily check for zero or blank in the demoninator. Example:<br>`IF(y <> 0, x / y)` -> `DIVIDE(x, y)` |
