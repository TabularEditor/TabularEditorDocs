...
Notice how the values in the **Locals** view are updated as the tree is navigated.
...
- **Debugging is currently not supported against Analysis Services on SQL Server 2016** due to the lack of support for the [TREATAS](https://dax.guide/TREATAS/) function.
...
- Auto-complete in the Watch expression editor still has a few issues. For example, it doesn't recognize variables within the current scope, or columns within active row contexts (however, the evaluation of such expressions still works).
