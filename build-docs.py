...
def copy_api_content_for_build(lang: str) -> int:
    """Copy API source content from main content folder to localized content folder before build.
    
    The API YAML files are generated by docfx metadata to content/api/ during the English build.
    This function copies them to localized language folders so they can build with proper toc.json.
    """
    # API content is generated to content/api/ by docfx metadata (not localizedContent/en/content/api/)
    api_content_src = Path("content/api")
    dest_api_content = Path(f"localizedContent/{lang}/content/api")
    
    if not api_content_src.exists():
        print(f"Warning: API content not found at {api_content_src}")
        return 0
    ...
    # Copy API content
    shutil.copytree(api_content_src, dest_api_content)
